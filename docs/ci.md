...
`fixed` 是**模型自己说的**。它可能真修好了这个 issue，但别处的测试还红着；也可能
测都没测就交了 `true`。实跑中就出现过：模型如实报告 issue 已修复，同时说明另一个老
bug 让一条测试仍然失败——结论没错，但仓库并不是绿的。

所以开 PR 之前，由 **workflow 自己**跑一遍测试（样本里是 `独立验证` 一步）：

- 测试命令和测试文件路径放在 job 级 `env`（`TEST_CMD` / `TEST_PATHSPEC`），拷到别的仓库
  只改这两行；
- 通过才开 PR；没通过就回帖，附测试输出末尾，并让这次运行**标红**；
- **默认分支本身要是绿的**。已有失败会让每次验证都不过——要么先修绿，要么把
  `TEST_CMD` 收窄到相关的测试。

模型开着 `--yolo`，**也能把测试改松让它"通过"**。样本会找出这次改动过的已有测试文件，
在 PR 描述里点名。刻意不做"验证前把测试恢复成基线"——那会连模型对测试的正当修改一起
丢掉。

要清楚这一步的边界：验证跑在模型刚用过的同一台机器上，它**不是**防恶意模型的安全边界
（模型真想作弊，能改的不止测试文件）。它挡的是"没修好就开 PR"这类诚实失误；防线仍然是
上面的凭证分离加 PR review。
