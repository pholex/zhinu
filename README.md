...
- **多 agent 织造**：声明式 subagent（`agents/*.toml` 放一个文件就多一个可委托的子 agent，带 worktree 隔离与 resume 续跑）；**七襄·并行织造模式**（Qixiang · Parallel-Weave）——召集多名织手横向并行，一个模板 + N 份材料扇出，按输入顺序聚合成带续跑句柄的 report；**宸枢·统筹织造模式**（Chenshu · Sovereign-Weave）——总枢坐镇规划、分派、监督、汇总，大工程拆成 scope 互不重叠的 mission，worker 在独立分支 worktree 里并行推进，评审过闸后合回主干。见 [docs/multi-agent.md](docs/multi-agent.md)
