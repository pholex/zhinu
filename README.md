...
- **多 agent 协同**：声明式 subagent（`agents/*.toml` 放一个文件就多一个可委托的子 agent，带 worktree 隔离与 resume 续跑）；**七襄**（qixiang）批量并行委托——一个模板 + N 份材料扇出并行跑，按输入顺序聚合成带续跑句柄的 report；**宸枢**（chenshu）编排总控——大工程拆成 scope 互不重叠的 mission，worker 在独立分支 worktree 里并行推进，评审过闸后合回主干。见 [docs/multi-agent.md](docs/multi-agent.md)
