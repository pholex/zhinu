...
> | 要加的东西 | 通道 | 落盘位置 | 生效时机 |
> |---|---|---|---|
> | 知识 / 流程 / 提示文本 | 技能（SKILL.md） | `~/.agents/skills/<名>/` | **落盘即可用**（详见技能小节） |
...
- 生效时机（三级，都不用重启）：**刚写完的技能当轮就能 `skill` 按名加载**
  （工具未命中时会重扫磁盘——以磁盘为真）；会话中途外部装入的技能在**下一轮
  开头**被自动检测并提示给模型；system prompt 里的索引默认下次会话刷新，
  想立刻刷新用 `/skills reload`（代价是 prompt cache 前缀作废一次）。
