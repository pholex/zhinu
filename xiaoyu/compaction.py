...
输入若含【此前的压缩摘要】节：那是上一轮的交接说明，不要把它当普通对话重新压缩一遍
——交接说明是持续维护的状态文档，不是每次重写的读后感。其中的信息默认全部保留
（用户消息清单与关键事实一条不丢），在它的基础上用【之后的新内容】增量更新：
已完成的事项挪进对应节、当前状态与下一步按新进展改写，确已失效的条目才可以删。
...
以上对话的开头若已含一份交接摘要（以"[以下是另一个模型对本会话早期内容做的交接摘要"
开头的段落）：那是上一轮的交接说明，不要把它当普通对话重新压缩一遍——交接说明是
持续维护的状态文档，不是每次重写的读后感。其中的信息默认全部保留（用户消息清单与
关键事实一条不丢），在它的基础上用之后的新对话增量更新：已完成的事项挪进对应节、
当前状态与下一步按新进展改写，确已失效的条目才可以删。
